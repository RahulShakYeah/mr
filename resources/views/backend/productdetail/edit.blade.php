        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="box box-primary">
                    <form method="post" action="{{route('productdetail.update',$productDetail->id)}}">
                        @csrf
                                ...
                                <select name="product_name" id="product_name" class="form-control">
                                    ...
                                    @if(count($product) > 0)
                                        @foreach($product as $key=>$value)
                                                <option value="{{$value->id}}" {{$productDetail->product_id == $value->id?'selected':''}}>{{$value->product_name}}</option>
                                        @endforeach
                                    @endif
                                </select>
                                @error('product_name')
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-12 col-md-9"><textarea name="product_detail" id="summernote" rows="9"
                                                                   class="form-control form-control-sm js-summernote" style="resize: none">{!! html_entity_decode($productDetail->product_details) !!}</textarea>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select name="status" id="status" class="form-control">
                                    <option value="active" {{$productDetail->status == "active"?"selected":""}}>Publish</option>
                                    <option value="inactive" {{$productDetail->status == "inactive"?"selected":""}}>Un-Publish</option>
                                </select>
                                @error('status')
                                <p class="alert-danger">{{$message}}</p>
                                @enderror
